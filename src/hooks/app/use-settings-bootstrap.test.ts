...
vi.mock("@/lib/settings", () => ({
  ...
  loadCopilotBudgetUsd: loadCopilotBudgetUsdMock,
...
describe("useSettingsBootstrap", () => {
  beforeEach(() => {
    ...
    loadCopilotBudgetUsdMock.mockReset()
    ...
    loadCopilotBudgetUsdMock.mockResolvedValue(40)
  ...
  it("loads and applies stored copilot budget", async () => {
    const args = createArgs()
    loadCopilotBudgetUsdMock.mockResolvedValueOnce(55)

    renderHook(() => useSettingsBootstrap(args))

    await waitFor(() => {
      expect(args.setCopilotBudgetUsd).toHaveBeenCalledWith(55)
    })
  })
})
